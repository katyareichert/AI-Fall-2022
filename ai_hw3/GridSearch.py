...
class GridSearch:
    ...
    def start(self) -> int:
        ...
        return self.grid.getMaxTile(), self.intelligentAgent.grid_score(self.grid), self.grid

def main():

    #f = open('gridsearch.csv', 'w')
    #writer = csv.writer(f)
    #header = ['w1', 'w2', 'w3', 'w4', 'w5', 'w6', 'utility', 'full_grid', 'max_tile']
    ...
    for i in range(11):
        gameManager = GridSearch(4, intelligentAgent, computerAI, displayer)
        (maxTile, utility, grid)     = gameManager.start()
        ...
        #if row[-1] > 1023:
        print("{}:  {}, {}, {}".format(i, round(utility, 3), full_grid, maxTile))
        #writer.writerow(row)
